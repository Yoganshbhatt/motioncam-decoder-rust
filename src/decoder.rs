...
impl Decoder {
    ...
    /// Hint the OS to prefetch the next frame's compressed bytes.
    ///
    /// Uses `madvise(MADV_WILLNEED)` on Unix to bring the frame's range
    /// into the page cache ahead of the actual read. No-op on Windows.
    #[cfg(unix)]
